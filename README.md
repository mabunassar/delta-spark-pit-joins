# Delta Lake Point-In-Time Join Example

This repository shows how to perform point-in-time (PIT) joins using Delta Lake with PySpark. The script `pit_join.py` demonstrates how to upsert feature data into a Delta table and then join those features to an observations table at the correct point in time.

## Requirements

- Python 3.9+
- [PySpark](https://spark.apache.org/docs/latest/api/python/index.html)
- [delta-spark](https://docs.delta.io/latest/delta-spark.html)
...
This will create temporary Delta tables under `/tmp/delta-table`, perform a merge update on the feature table, and show the output of the point-in-time join.

---

Below are sample tables produced by running the script.
...
I then performed a "Merge"/"Upsert" where we update all columns unless all columns (except timestamp) of the new potential entry match an existing row, in which case we only update the timestamp column.

Here's the table after updating it with the `batch_data` specified in `FeaturesTable`:
```
+----------+-----------+-------------------+----------------------------------+---------------------------------+---------------------------------+---------------------------------+
|account_id| contact_id|          timestamp|f_contact_click_count_over_30_days|f_contact_click_count_over_60_days|f_contact_open_count_over_30_days|f_contact_open_count_over_60_days|
+----------+-----------+-------------------+----------------------------------+---------------------------------+---------------------------------+---------------------------------+
...
Now it's time to do the point-in-time join where our left table is the observations table and for each observation we bring the most recent feature computed:
```
+----------+----------+-------------------+----------+----------+----------+-------------------+----------------------------------+---------------------------------+---------------------------------+---------------------------------+
